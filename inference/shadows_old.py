...
if __name__ == "__main__":
    ...
    #COMBINED_DEPTH_MAP_PATH = "./output/shampoo-cycling/cycling-8215973_shampoo-1860642_combined_depth.png"
    #COMBINED_NAME = "cycling-8215973_shampoo-1860642" # os.path.basename(COMBINED_DEPTH_MAP_PATH).split(".")[0]

    COMBINED_DEPTH_MAP_PATH = "./examples/cone_chair/composite_depth.png"
    COMBINED_NAME = "chair" # os.path.basename(COMBINED_DEPTH_MAP_PATH).split(".")[0]

    #ORIGINAL_DEPTH_MAP = ""
    
    #COMPOSITE_MASK = "./output/shampoo-cycling/shampoo-1860642_full_mask.png"
    #COMPOSITE_NAME = "shampoo-1860642_mask"
    
    COMPOSITE_MASK = "./examples/cone_chair/mask.png"
    COMPOSITE_NAME = "chair_mask"
    
    COMPOSITE_SCALE = 0.25 # how large the fg image should be when compared to the bg
    COMPOSITE_LOCATION = [0.0, 0.0]
    
    # --------------------------------------------------- #
    
    print("1. load & generate our images")
    combined_depth_map = load_image(COMBINED_DEPTH_MAP_PATH)
    full_size_mask = load_image(COMPOSITE_MASK)
    #fg_mask_rescaled = load_image(COMBINED_NAME)
    ...
    light_direction = np.asarray([30, 30, -1])
    ...
    print("4. save output")
    np_to_pil(full_size_mask).save(f"{COMPOSITE_NAME}_full_mask.png")

    np_to_pil(shaded_mask).save(f"{COMBINED_NAME}_shaded_mask.png")
